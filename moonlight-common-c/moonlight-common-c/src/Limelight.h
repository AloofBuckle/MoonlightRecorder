...
// If set in the audio renderer capabilities field, Moonlight will preserve the initial
// audio packets instead of dropping the startup buffer for real-time playback resync.
// This is intended for capture and remux use cases that need an untrimmed audio timeline.
#define CAPABILITY_NO_STARTUP_AUDIO_DROP 0x80
...
// receiveTimeUs is the local receive timestamp in microseconds for the packet payload.
// rtpTimestamp is the audio RTP timestamp in host byte order. Audio RTP timestamps
// advance in AudioPacketDuration-millisecond increments on Sunshine/GFE hosts.
