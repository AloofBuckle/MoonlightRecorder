...
static void decodeInputData(PQUEUED_AUDIO_PACKET packet) {
    ...
    if (packet->header.size == 0) {
        AudioCallbacks.decodeAndPlaySample(NULL, 0, 0, 0);
        return;
    }
    ...
    if (AudioEncryptionEnabled) {
        ...
        AudioCallbacks.decodeAndPlaySample((char*)decryptedOpusData,
                                           dataLength,
                                           packet->header.receiveTimeUs,
                                           rtp->timestamp);
    }
    else {
#ifdef LC_DEBUG
        ...
        AudioCallbacks.decodeAndPlaySample((char*)(rtp + 1),
                                           packet->header.size - sizeof(*rtp),
                                           packet->header.receiveTimeUs,
                                           rtp->timestamp);
    }
}
...
static void AudioReceiveThreadProc(void* context) {
    ...
    if ((AudioCallbacks.capabilities & CAPABILITY_NO_STARTUP_AUDIO_DROP) != 0) {
        packetsToDrop = 0;
    }
    ...
    while (!PltIsThreadInterrupted(&receiveThread)) {
        ...
        packet->header.receiveTimeUs = LiGetMicroseconds();
        if (packet->header.size < 0) {
