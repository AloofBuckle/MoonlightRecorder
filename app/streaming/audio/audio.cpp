...
int Session::getAudioRendererCapabilities(int audioConfiguration)
{
    ...
    if (isBitstreamPassthroughMode()) {
        caps |= CAPABILITY_NO_STARTUP_AUDIO_DROP;
    }
                    ...
                    void* /* arContext */, int /* arFlags */)
{
    ...
    SDL_memcpy(&s_ActiveSession->m_ActiveAudioConfig, opusConfig, sizeof(*opusConfig));

    if (s_ActiveSession->isBitstreamPassthroughMode()) {
        s_ActiveSession->ensureBitstreamMuxer()->setAudioConfig(*opusConfig);
        SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION,
                    "Bitstream audio passthrough active: %d channels at %d Hz",
                    opusConfig->channelCount,
                    opusConfig->sampleRate);
        return 0;
    }

    return s_ActiveSession->initializeAudioRenderer() ? 0 : -1;
}

void Session::arCleanup()
{
    if (s_ActiveSession->isBitstreamPassthroughMode()) {
        if (s_ActiveSession->m_BitstreamMuxer != nullptr) {
            s_ActiveSession->m_BitstreamMuxer->closeAudioInput();
        }
        return;
    }
...
void Session::arDecodeAndPlaySample(char* sampleData,
                                    int sampleLength,
                                    ...
                                    quint32 rtpTimestamp)
{
    if (s_ActiveSession->isBitstreamPassthroughMode()) {
        if (s_ActiveSession->m_BitstreamMuxer != nullptr &&
            !s_ActiveSession->m_BitstreamMuxer->writeAudioPacket(sampleData,
                                                                 sampleLength,
                                                                 receiveTimeUs,
                                                                 rtpTimestamp)) {
            SDL_LogError(SDL_LOG_CATEGORY_APPLICATION,
                         "Failed writing raw audio bitstream to FFmpeg muxer");
