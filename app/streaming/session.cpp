...
QString describeRecordingAudioMode()
{
    switch (StreamingPreferences::get()->recordingAudioMode) {
    case StreamingPreferences::RAM_AAC_COMPAT:
        return QObject::tr("AAC compatibility");
    case StreamingPreferences::RAM_OPUS_COPY:
    default:
        return QObject::tr("Opus copy");
    }
}

QString describeRecordingTarget(const QString& target)
{
    const QString containerName =
            StreamingPreferences::recordingContainerDisplayName(
                StreamingPreferences::get()->recordingContainer);
    const QString containerExtension =
            StreamingPreferences::recordingContainerExtension(
                StreamingPreferences::get()->recordingContainer);
    QString trimmedTarget = target.trimmed();
    if (trimmedTarget.isEmpty()) {
        return QObject::tr("Desktop (auto-named %1, %2)")
                .arg(containerName, describeRecordingAudioMode());
    }

    if (trimmedTarget.contains("://")) {
        return QObject::tr("%1 (%2)").arg(trimmedTarget, describeRecordingAudioMode());
    }

    QFileInfo targetInfo(trimmedTarget);
    if (trimmedTarget.endsWith('/') ||
        trimmedTarget.endsWith('\\') ||
        (targetInfo.exists() && targetInfo.isDir())) {
        QString directoryPath = targetInfo.exists() && targetInfo.isDir() ?
                    targetInfo.absoluteFilePath() :
                    QDir::cleanPath(trimmedTarget);
        return QObject::tr("%1 (auto-named %2, %3)")
                .arg(QDir::toNativeSeparators(directoryPath),
                     containerName,
                     describeRecordingAudioMode());
    }

    if (targetInfo.suffix().isEmpty()) {
        return QObject::tr("%1 (%2)")
                .arg(QDir::toNativeSeparators(trimmedTarget + "." + containerExtension),
                     describeRecordingAudioMode());
    }

    return QObject::tr("%1 (%2)")
            .arg(QDir::toNativeSeparators(trimmedTarget),
                 describeRecordingAudioMode());
}
                            ...
                            int frameRate, bool enableVsync, bool enableFramePacing, bool testOnly, IVideoDecoder*& chosenDecoder)
{
    ...
    if (StreamingPreferences::get()->enableBitstreamPassthrough) {
        chosenDecoder = new BitstreamVideoDecoder(testOnly);
        if (chosenDecoder->initialize(&params)) {
            SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION,
                        "Bitstream passthrough sink chosen");
            return true;
        }
        else {
            SDL_LogError(SDL_LOG_CATEGORY_APPLICATION,
                         "Unable to initialize bitstream passthrough sink");
...
bool Session::isBitstreamPassthroughMode() const
{
    return m_IsBitstreamSession;
}

QString Session::getRecordingSummary() const
{
    QByteArray overlayText = QByteArray(m_OverlayManager.getOverlayText(Overlay::OverlayDebug));
    if (!overlayText.isEmpty()) {
        return QString::fromUtf8(overlayText);
    }

    return tr("Waiting for live stream stats...");
}

QString Session::getRecordingOutputTarget() const
{
    return m_RecordingOutputTarget;
}

BitstreamMuxer* Session::ensureBitstreamMuxer()
{
    if (m_BitstreamMuxer == nullptr) {
        m_BitstreamMuxer = new BitstreamMuxer();

        if (m_OriginalAudioConfig.channelCount > 0) {
            m_BitstreamMuxer->setAudioConfig(m_OriginalAudioConfig);
        }
    }

    return m_BitstreamMuxer;
}

void Session::updateRecordingOverlayText(const char* text)
{
    m_OverlayManager.updateOverlayText(Overlay::OverlayDebug, text);
    emit recordingInfoChanged();
}
...
Session::Session(NvComputer* computer, NvApp& app, StreamingPreferences *preferences)
    : m_Preferences(preferences ? preferences : StreamingPreferences::get()),
      ...
      m_ShouldExit(false),
      m_IsBitstreamSession(m_Preferences->enableBitstreamPassthrough &&
                           !m_Preferences->bitstreamOutputTarget.isEmpty()),
      ...
      m_RecordingOutputTarget(describeRecordingTarget(m_Preferences->bitstreamOutputTarget)),
      m_BitstreamMuxer(nullptr)
{
    SDL_zero(m_StreamConfig);
}
...
bool Session::initialize(QQuickWindow* qtWindow)
{
    ...
    if (isBitstreamPassthroughMode()) {
        SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION,
                    "Bitstream passthrough mode enabled - local audio playback disabled, raw audio recording enabled");
    }
    ...
    if (isBitstreamPassthroughMode()) {
        updateRecordingOverlayText("Waiting for first video stats...");
        emit recordingInfoChanged();
    }
...
bool Session::validateBitstreamLaunch()
{
    emitLaunchWarning(tr("Experimental bitstream passthrough mode disables local video playback and local audio playback, and records the original video and audio bitstreams through FFmpeg."));

    if (m_Preferences->videoDecoderSelection != StreamingPreferences::VDS_AUTO) {
        emitLaunchWarning(tr("Local decoder selection is ignored in bitstream passthrough mode."));
    }
    ...
    if (m_SupportedVideoFormats & VIDEO_FORMAT_MASK_AV1) {
        ...
        else if (m_Preferences->videoCodecConfig == StreamingPreferences::VCC_FORCE_AV1) {
            m_SupportedVideoFormats.removeByMask(~VIDEO_FORMAT_MASK_AV1);
        }
    }
    ...
    if (m_SupportedVideoFormats & VIDEO_FORMAT_MASK_H265) {
        ...
        else if (m_Preferences->videoCodecConfig == StreamingPreferences::VCC_FORCE_HEVC) {
            m_SupportedVideoFormats.removeByMask(~VIDEO_FORMAT_MASK_H265);
        }
    }
    ...
    if (m_Preferences->enableHdr) {
        ...
        else if ((m_Computer->serverCodecModeSupport & SCM_MASK_10BIT) == 0) {
    ...
    if (m_Preferences->enableYUV444) {
        if (!(m_Computer->serverCodecModeSupport & SCM_MASK_YUV444)) {
            ...
            int serverCompatibleFormats = m_SupportedVideoFormats.maskByServerCodecModes(m_Computer->serverCodecModeSupport);
            if (serverCompatibleFormats != 0) {
                m_SupportedVideoFormats.removeByMask(~serverCompatibleFormats);
            }
    ...
    // Keep the existing host-side encoder capability checks for >4K video.
...
bool Session::validateLaunch(SDL_Window* testWindow)
{
    ...
    if (isBitstreamPassthroughMode()) {
        return validateBitstreamLaunch();
    }
...
bool Session::startConnectionAsync()
{
    ...
    try {
        int gamepadMask = m_InputHandler != nullptr ? m_InputHandler->getAttachedGamepadMask() : 0;
...
void Session::execBitstreamPassthrough()
{
    ...
    SDL_LockMutex(m_DecoderLock);
    bool decoderReady = chooseDecoder(m_Preferences->videoDecoderSelection,
                                      nullptr,
                                      m_ActiveVideoFormat,
                                      m_ActiveVideoWidth,
                                      m_ActiveVideoHeight,
                                      ...
                                      false,
                                      m_VideoDecoder);
    if (decoderReady && m_VideoDecoder != nullptr) {
        m_VideoDecoder->setHdrMode(LiGetCurrentHostDisplayHdrMode());
    }
    SDL_UnlockMutex(m_DecoderLock);

    if (!decoderReady) {
        emit displayLaunchError(tr("Unable to initialize FFmpeg bitstream sink. Please check your FFmpeg path and output arguments."));
        finishExec(nullptr);
        return;
    }

    SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION,
                "Bitstream passthrough active for %dx%dx%d format 0x%x",
                m_ActiveVideoWidth,
                m_ActiveVideoHeight,
                m_ActiveVideoFrameRate,
                m_ActiveVideoFormat);
    ...
    // Ask the host for a fresh keyframe so the sink starts on a clean boundary.
    LiRequestIdrFrame();
    ...
    for (;;) {
        ...
        if (event.type == SDL_QUIT) {
            SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION,
                        "Bitstream passthrough session ending");
            if (m_BitstreamMuxer != nullptr) {
                m_BitstreamMuxer->shutdown();
            }
            break;
        }
    }
...
void Session::exec()
{
    ...
    if (!m_AsyncConnectionSuccess) {
        m_Preferences->clearBitstreamPassthrough();
    ...
    for (;;) {
        ...
        case SDL_QUIT:
            SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION,
                        "Quit event received");
            if (isBitstreamPassthroughMode() && m_BitstreamMuxer != nullptr) {
                m_BitstreamMuxer->shutdown();
            }
...
DispatchDeferredCleanup:
    finishExec(iconSurface);
}

void Session::finishExec(SDL_Surface* iconSurface)
{
    m_Preferences->clearBitstreamPassthrough();
    ...
    // before allowing the UI to continue execution or it could
