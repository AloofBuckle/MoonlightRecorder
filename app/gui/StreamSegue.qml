...
Item {
    ...
    property string stageText : showRecordingPanel ?
                                    (isResume ? qsTr("Preparing recording for %1...").arg(appName) :
                                                qsTr("Starting recording for %1...").arg(appName)) :
                                    (isResume ? qsTr("Resuming %1...").arg(appName) :
                                                qsTr("Starting %1...").arg(appName))
    property bool isResume : false
    property bool quitAfter : false
    property bool recordingActive : false
    property bool recordingInfoReady : false
    property bool showRecordingPanel : session && session.bitstreamPassthroughMode
    ...
    Keys.onCancelPressed: function(event) {
        cancelAndReturn(event)
    }
    ...
    Row {
        ...
        spacing: 5
        visible: !showRecordingPanel || !recordingInfoReady
    ...
    Rectangle {
        ...
        height: recordingCardContent.implicitHeight + 48
        radius: 18
        color: "#202020"
        border.color: "#505050"
        border.width: 1
        ...
        Column {
            id: recordingCardContent
            width: recordingCard.width - 48
            ...
            Row {
                ...
                Rectangle {
                    ...
                    height: 14
                    radius: 7
                    color: recordingActive ? "#d84d4d" : "#d0a030"
                }

                Label {
                    width: parent.width - 26
                    text: recordingActive ? qsTr("Recording") : qsTr("Preparing Recording...")
                    font.pointSize: 22
                    font.bold: true
                }
            }
            ...
            Label {
                width: parent.width
                text: qsTr("Live stats below reuse Moonlight's existing in-stream debug text.")
                wrapMode: Text.Wrap
                color: "#d0d0d0"
            }
            ...
            Rectangle {
                width: parent.width
                height: recordingStatsText.implicitHeight + 24
                radius: 12
                color: "#111111"
                border.color: "#383838"
                border.width: 1
                ...
                Label {
                    ...
                    text: session ? session.recordingSummary : ""
                    wrapMode: Text.Wrap
            ...
            Label {
                width: parent.width
                text: qsTr("Output: %1").arg(session ? session.recordingOutputTarget : "")
                wrapMode: Text.Wrap
            }
            ...
            Row {
                ...
                Button {
                    text: recordingActive ? qsTr("End Recording and Return") : qsTr("Cancel")
                    onClicked: {
                        cancelAndReturn(null)
                    }
                }

                Label {
                    anchors.verticalCenter: parent.verticalCenter
                    text: qsTr("Hotkey: %1").arg(qsTr("Ctrl+Alt+Shift+Q"))
                    color: "#bcbcbc"
                }
            }
        }
