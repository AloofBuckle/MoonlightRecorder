...
Flickable {
    id: settingsPage
    objectName: qsTr("Recording Settings")
    ...
    Column {
        ...
        GroupBox {
            ...
            padding: 12
            title: "<font color=\"skyblue\">" + qsTr("Recording Profile") + "</font>"
            font.pointSize: 12
            ...
            Column {
                ...
                Label {
                    width: parent.width
                    id: resFPSdesc
                    text: qsTr("Choose the target capture profile sent to the host while recording.")
                    font.pointSize: 9
                    wrapMode: Text.Wrap
                }
                ...
                Label {
                    width: parent.width
                    id: bitrateDesc
                    text: qsTr("Lower the bitrate for unstable networks. Raise it to preserve more source quality in the recorded file.")
                    font.pointSize: 9
                    wrapMode: Text.Wrap
                }
                ...
                Row {
                    ...
                    Slider {
                        ...
                        onValueChanged: {
                            bitrateTitle.text = qsTr("Recording bitrate: %1 Mbps").arg(value / 1000.0)
        ...
        GroupBox {
            ...
            width: (parent.width - (parent.leftPadding + parent.rightPadding))
            padding: 12
            title: "<font color=\"skyblue\">" + qsTr("Audio Capture") + "</font>"
            font.pointSize: 12
            ...
            Column {
                ...
                Label {
                    width: parent.width
                    text: qsTr("Choose how Moonlight packages audio into the recorded file. Opus copy keeps the original host audio stream. AAC compatibility re-encodes audio for broader player/container compatibility and uses the FFmpeg output path.")
                    font.pointSize: 9
                    wrapMode: Text.Wrap
                }
                ...
                AutoResizingComboBox {
                    Component.onCompleted: {
                        var savedAudioMode = StreamingPreferences.recordingAudioMode
                        currentIndex = 0
                        for (var i = 0; i < recordingAudioModeListModel.count; i++) {
                            if (recordingAudioModeListModel.get(i).val === savedAudioMode) {
                                currentIndex = i
                                break
                            }
                        }
                    ...
                    id: recordingAudioModeComboBox
                    textRole: "text"
                    model: ListModel {
                        id: recordingAudioModeListModel
                        ListElement {
                            text: qsTr("Original Opus (Source-Exact)")
                            val: StreamingPreferences.RAM_OPUS_COPY
                        }
                        ListElement {
                            text: qsTr("AAC (Compatibility)")
                            val: StreamingPreferences.RAM_AAC_COMPAT
                        }
                    }

                    onActivated: {
                        StreamingPreferences.recordingAudioMode =
                                recordingAudioModeListModel.get(currentIndex).val
                    }
                }

                Label {
                    width: parent.width
                    text: StreamingPreferences.recordingAudioMode === StreamingPreferences.RAM_OPUS_COPY ?
                              qsTr("Opus copy effect: preserves the original Sunshine audio bitstream and original channel layout, but playback compatibility depends on the chosen container/player.")
                            :
                              qsTr("AAC effect: audio is re-encoded during capture for better compatibility with MP4/MOV/streaming workflows, but it is no longer bit-for-bit identical to the host stream.")
                    font.pointSize: 9
                    wrapMode: Text.Wrap
                    color: StreamingPreferences.recordingAudioMode === StreamingPreferences.RAM_OPUS_COPY ? "#a8d8a8" : "#e0d080"
                }
        ...
        GroupBox {
            id: outputSettingsGroupBox
            width: (parent.width - (parent.leftPadding + parent.rightPadding))
            padding: 12
            title: "<font color=\"skyblue\">" + qsTr("Output") + "</font>"
            font.pointSize: 12
            ...
            Column {
                ...
                Label {
                    width: parent.width
                    text: qsTr("Choose the file container used for new local recordings.")
                    font.pointSize: 9
                    wrapMode: Text.Wrap
                }

                AutoResizingComboBox {
                    id: recordingContainerComboBox
                    textRole: "text"

                    Component.onCompleted: {
                        currentIndex = 0
                        for (var i = 0; i < recordingContainerListModel.count; i++) {
                            if (recordingContainerListModel.get(i).val === StreamingPreferences.recordingContainer) {
                                currentIndex = i
                                break
                            }
                        }
                    ...
                    model: ListModel {
                        id: recordingContainerListModel
                        ListElement {
                            text: qsTr("MKV (Recommended)")
                            val: StreamingPreferences.RC_MKV
                        }
                        ListElement {
                            text: qsTr("MP4")
                            val: StreamingPreferences.RC_MP4
                        }
                        ListElement {
                            text: qsTr("MOV")
                            val: StreamingPreferences.RC_MOV
                        }
                        ListElement {
                            text: qsTr("MPEG-TS")
                            val: StreamingPreferences.RC_TS
                        }
                        ListElement {
                            text: qsTr("FLV")
                            val: StreamingPreferences.RC_FLV
                        }
                    }

                    onActivated: {
                        StreamingPreferences.recordingContainer = recordingContainerListModel.get(currentIndex).val
                    }
                }

                Label {
                    width: parent.width
                    text: qsTr("Recording directory")
                    font.pointSize: 12
                    wrapMode: Text.Wrap
                }
                ...
                Label {
                    width: parent.width
                    text: qsTr("New local recordings will be written into this folder.")
                    font.pointSize: 9
                    wrapMode: Text.Wrap
                }
                ...
                RowLayout {
                    ...
                    TextField {
                        ...
                        readOnly: true
                        selectByMouse: true
                        text: StreamingPreferences.recordingOutputDirectory
                    }

                    Button {
                        id: browseRecordingDirectoryButton
                        text: qsTr("Browse...")
                        onClicked: {
                            var selectedPath = StreamingPreferences.browseForRecordingOutputDirectory()
                            if (selectedPath.length > 0) {
                                StreamingPreferences.recordingOutputDirectory = selectedPath
                            }
                        }
                    }
                }
                ...
                Label {
                    width: parent.width
                    text: qsTr("New files will be named Moonlight-YYYYMMDD-HHMMSS.%1")
                              .arg(StreamingPreferences.recordingContainerExtension(StreamingPreferences.recordingContainer))
                    font.pointSize: 9
                    wrapMode: Text.Wrap
                }
            }
    ...
    Column {
        ...
        GroupBox {
            ...
            width: (parent.width - (parent.leftPadding + parent.rightPadding))
            padding: 12
            title: "<font color=\"skyblue\">" + qsTr("Recorder Options") + "</font>"
            font.pointSize: 12
            ...
            Column {
                ...
                Label {
                    width: parent.width
                    id: resVCCTitle
                    text: qsTr("Recorded video codec")
                    font.pointSize: 12
                    wrapMode: Text.Wrap
                }
                ...
                Label {
                    width: parent.width
                    id: hevcRecordingIdrTitle
                    text: {
                        var interval = StreamingPreferences.hevcRecordingIdrIntervalMs
                        if (interval <= 0) {
                            return qsTr("Periodic HEVC compatibility IDR requests: Off (Default)")
                        }

                        return qsTr("Periodic HEVC compatibility IDR requests: Every %1")
                                   .arg(hevcRecordingIdrControls.formatInterval(interval))
                    }
                    font.pointSize: 12
                    wrapMode: Text.Wrap
                }
                ...
                Label {
                    width: parent.width
                    id: hevcRecordingIdrDesc
                    text: qsTr("This option exposes the underlying HEVC seek-compatibility workaround directly. When enabled, Moonlight periodically asks the host for extra HEVC IDR keyframes while recording. That usually improves VLC progress-bar behavior and seeking reliability, but it also creates larger bitrate spikes and can cause visible periodic refresh/softening in the picture. Leave it off unless you specifically need better HEVC seeking/player compatibility.")
                    font.pointSize: 9
                    wrapMode: Text.Wrap
                }

                Column {
                    id: hevcRecordingIdrControls
                    width: parent.width
                    spacing: 8

                    property int minIntervalMs: 250
                    property int maxIntervalMs: 5000
                    property int recommendedIntervalMs: 1000

                    function clampInterval(intervalMs) {
                        return Math.max(minIntervalMs, Math.min(maxIntervalMs, intervalMs))
                    }

                    function sliderValueFromInterval(intervalMs) {
                        var clamped = clampInterval(intervalMs)
                        return Math.log(clamped / minIntervalMs) / Math.log(maxIntervalMs / minIntervalMs)
                    }

                    function intervalFromSliderValue(sliderValue) {
                        var rawInterval = minIntervalMs * Math.pow(maxIntervalMs / minIntervalMs, sliderValue)
                        var roundedInterval

                        if (rawInterval < 1000) {
                            roundedInterval = Math.round(rawInterval / 25) * 25
                        }
                        else if (rawInterval < 2000) {
                            roundedInterval = Math.round(rawInterval / 50) * 50
                        }
                        else {
                            roundedInterval = Math.round(rawInterval / 100) * 100
                        }

                        return clampInterval(roundedInterval)
                    }
                    ...
                    function formatInterval(intervalMs) {
                        if (intervalMs >= 1000) {
                            ...
                            if (Math.abs(seconds - Math.round(seconds)) < 0.001) {
                                ...
                                return wholeSeconds === 1 ? qsTr("1 second")
                                                          : qsTr("%1 seconds").arg(wholeSeconds)
                            }

                            return qsTr("%1 seconds").arg(seconds.toFixed(2).replace(/0+$/, "").replace(/\.$/, ""))
                        }

                        return qsTr("%1 ms").arg(intervalMs)
                    }

                    function applyInterval(intervalMs) {
                        var clampedInterval = clampInterval(intervalMs)
                        StreamingPreferences.hevcRecordingIdrIntervalMs = clampedInterval
                        hevcIdrSlider.value = sliderValueFromInterval(clampedInterval)
                    }

                    Component.onCompleted: {
                        var initialInterval = StreamingPreferences.hevcRecordingIdrIntervalMs
                        if (initialInterval <= 0) {
                            initialInterval = recommendedIntervalMs
                        }
                        hevcIdrSlider.value = sliderValueFromInterval(initialInterval)
                    }

                    CheckBox {
                        id: hevcIdrEnableCheck
                        width: parent.width
                        text: qsTr("Enable extra HEVC IDR keyframes for VLC/seek compatibility")
                        font.pointSize: 12
                        checked: StreamingPreferences.hevcRecordingIdrIntervalMs > 0

                        onCheckedChanged: {
                            if (!checked) {
                                StreamingPreferences.hevcRecordingIdrIntervalMs = 0
                            }
                            else if (StreamingPreferences.hevcRecordingIdrIntervalMs <= 0) {
                                hevcRecordingIdrControls.applyInterval(hevcRecordingIdrControls.recommendedIntervalMs)
                            }
                        }
                    }
                    ...
                    Label {
                        width: parent.width
                        text: hevcIdrEnableCheck.checked ?
                                  qsTr("Enabled effect: better HEVC seeking and more stable VLC progress-bar updates, but expect extra bitrate spikes and possible periodic picture refresh/softening when a forced IDR arrives.")
                                :
                                  qsTr("Disabled effect: cleanest picture and best compression efficiency, but some HEVC files may seek less reliably in VLC or other stricter players.")
                        font.pointSize: 9
                        wrapMode: Text.Wrap
                        color: hevcIdrEnableCheck.checked ? "#e0d080" : "#a8d8a8"
                    }
                    ...
                    Row {
                        ...
                        Slider {
                            id: hevcIdrSlider
                            enabled: hevcIdrEnableCheck.checked
                            from: 0
                            to: 1
                            value: hevcRecordingIdrControls.sliderValueFromInterval(hevcRecordingIdrControls.recommendedIntervalMs)
                            width: Math.min(hevcRecordingIdrDesc.implicitWidth,
                                            parent.width - (hevcIdrResetButton.visible ? hevcIdrResetButton.width + parent.spacing : 0))

                            onMoved: {
                                if (enabled) {
                                    hevcRecordingIdrControls.applyInterval(
                                                hevcRecordingIdrControls.intervalFromSliderValue(value))
                                }
                            }
                            ...
                            ToolTip.visible: hovered
                            ToolTip.text: qsTr("Only affects new HEVC bitstream recordings. Lower values request extra IDR frames more often for player compatibility, but increase visible quality pulsing risk.")
                        }

                        Button {
                            id: hevcIdrResetButton
                            text: hevcIdrEnableCheck.checked ?
                                      qsTr("Use 1 second preset")
                                    :
                                      qsTr("Enable 1 second preset")
                            visible: !hevcIdrEnableCheck.checked ||
                                     StreamingPreferences.hevcRecordingIdrIntervalMs !== hevcRecordingIdrControls.recommendedIntervalMs
                            onClicked: {
                                hevcIdrEnableCheck.checked = true
                                hevcRecordingIdrControls.applyInterval(hevcRecordingIdrControls.recommendedIntervalMs)
                            }
                        }
                    }
                ...
                CheckBox {
                    id: enableHdr
                    width: parent.width
                    text: qsTr("Record HDR metadata (Experimental)")
                    font.pointSize: 12
                ...
                CheckBox {
                    id: enableYUV444
                    width: parent.width
                    text: qsTr("Request YUV 4:4:4 capture (Experimental)")
                    font.pointSize: 12
                ...
                CheckBox {
                    id: unlockBitrate
                    width: parent.width
                    text: qsTr("Unlock high bitrate ceiling (Experimental)")
                    font.pointSize: 12
                ...
                CheckBox {
                    id: enableMdns
                    width: parent.width
                    text: qsTr("Automatically discover recording hosts (Recommended)")
                    font.pointSize: 12
                ...
                CheckBox {
                    ...
                    width: parent.width
                    text: qsTr("Show recording stats during capture")
                    font.pointSize: 12
                    ...
                    ToolTip.timeout: 5000
                    ToolTip.visible: hovered
                    ToolTip.text: qsTr("Display real-time capture information while recording.") + "\n\n" +
                                  qsTr("You can toggle it at any time while recording using Ctrl+Alt+Shift+S or Select+L1+R1+X.") + "\n\n" +
