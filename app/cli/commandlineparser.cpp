...
void StreamCommandLineParser::parse(const QStringList &args, StreamingPreferences *preferences)
{
    ...
    parser.addValueOption("bitstream-output", "ffmpeg output path or URL for experimental bitstream passthrough mode");
    parser.addValueOption("ffmpeg-path", "custom ffmpeg executable path");
    parser.addValueOption("ffmpeg-extra-args", "extra ffmpeg arguments inserted before the output target");
    ...
    // Stream parser state is reused through the global preferences singleton.
    // Reset our experimental CLI-only values before applying overrides.
    preferences->enableBitstreamPassthrough = false;
    preferences->bitstreamOutputTarget.clear();
    preferences->ffmpegPath.clear();
    preferences->ffmpegExtraArgs.clear();
    ...
    if (parser.isSet("bitstream-output")) {
        preferences->enableBitstreamPassthrough = true;
        preferences->bitstreamOutputTarget = parser.value("bitstream-output").trimmed();
        if (preferences->bitstreamOutputTarget.isEmpty()) {
            parser.showError("Bitstream output target must not be empty");
        }
    }

    if (parser.isSet("ffmpeg-path")) {
        preferences->ffmpegPath = parser.value("ffmpeg-path").trimmed();
        if (preferences->ffmpegPath.isEmpty()) {
            parser.showError("FFmpeg path must not be empty");
        }
    }

    if (parser.isSet("ffmpeg-extra-args")) {
        preferences->ffmpegExtraArgs = parser.value("ffmpeg-extra-args").trimmed();
    }
