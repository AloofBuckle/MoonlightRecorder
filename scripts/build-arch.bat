...
if /I "%ARCH%" EQU "x64" (
    ...
    call "%VS_INSTALL_DIR%\VC\Auxiliary\Build\vcvarsall.bat" %VC_ARCH%
)
...
echo Compiling %APP_NAME% in %BUILD_CONFIG% configuration
pushd %BUILD_FOLDER%
...
if !ERRORLEVEL! NEQ 0 goto Error
powershell -NoProfile -Command "Compress-Archive -Path '%SYMBOLS_FOLDER%\*.pdb' -DestinationPath '%SYMBOLS_FOLDER%\%APP_ID%DebuggingSymbols-%ARCH%-%VERSION%.zip' -Force"
if !ERRORLEVEL! NEQ 0 goto Error

if "%ML_SYMBOL_STORE%" NEQ "" (
    echo Publishing PDBs to symbol store: %ML_SYMBOL_STORE%
    symstore add /f %SYMBOLS_FOLDER%\*.pdb /s %ML_SYMBOL_STORE% /t "%APP_NAME%"
    if !ERRORLEVEL! NEQ 0 goto Error
) else (
...
if "%ML_SYMBOL_ARCHIVE%" NEQ "" (
    ...
    copy %SYMBOLS_FOLDER%\%APP_ID%DebuggingSymbols-%ARCH%-%VERSION%.zip %ML_SYMBOL_ARCHIVE%
    if !ERRORLEVEL! NEQ 0 goto Error
) else (
    if "%MUST_DEPLOY_SYMBOLS%"=="1" (
...
if "%ML_SYMBOL_STORE%" NEQ "" (
    ...
    symstore add /r /f %DEPLOY_FOLDER%\*.* /s %ML_SYMBOL_STORE% /t "%APP_NAME%"
    if !ERRORLEVEL! NEQ 0 goto Error
    symstore add /r /f %BUILD_FOLDER%\app\%BUILD_CONFIG%\%APP_EXE% /s %ML_SYMBOL_STORE% /t "%APP_NAME%"
    if !ERRORLEVEL! NEQ 0 goto Error
)
...
powershell -NoProfile -Command "Compress-Archive -Path '%DEPLOY_FOLDER%\*' -DestinationPath '%INSTALLER_FOLDER%\%APP_ID%Portable-%ARCH%-%VERSION%.zip' -Force"
if !ERRORLEVEL! NEQ 0 goto Error

echo Build successful for %APP_NAME% v%VERSION% %ARCH% binaries!
exit /b 0
